#!/usr/bin/env python3
"""
Validators - Check if GIFs meet Discord's requirements.

These validators help ensure your GIFs meet Discord's size and dimension constraints.
"""
...
def validate_gif(
    ...
    """
    Validate GIF for Discord (dimensions, size, frame count).
    ...
    if verbose:
        ...
        if not dim_pass:
            print(
                f"  Note: {'Emoji should be 128x128' if is_emoji else 'Unusual dimensions for Discord'}"
            )
...
def is_discord_ready(
    ...
    """
    Quick check if GIF is ready for Discord.
